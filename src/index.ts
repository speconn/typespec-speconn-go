...
function emitGo(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    client.push(`func New${svc.serviceName}Client(httpClient *http.Client, baseURL string) ${svc.serviceName}Client {`);
    ...
    for (const rpc of svc.rpcs) {
      if (rpc.isStream) {
        client.push(`\t${rpc.name} *speconn.StreamClient[${reqName(rpc)}, ${resName(rpc)}]`);
      } else {
        client.push(`\t${rpc.name} *speconn.Client[${reqName(rpc)}, ${resName(rpc)}]`);
      }
    }
    client.push('}\n');
    for (const rpc of svc.rpcs) {
