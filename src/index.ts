...
function emitGo(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    server.push('import "encoding/json"');
    server.push('import "io"');
    server.push('import "net/http"');
    server.push('import "github.com/speconn/speconn-go/speconn"\n');
    ...
    for (const rpc of svc.rpcs) {
      ...
      server.push(`\t\tcase ${procConst}:`);
      if (rpc.isStream) {
        server.push(`\t\t\tvar req ${reqName(rpc)}`);
        server.push(`\t\t\tif r.ContentLength > 0 { json.NewDecoder(r.Body).Decode(&req) }`);
        server.push(`\t\t\tflusher, _ := w.(http.Flusher)`);
        server.push(`\t\t\tw.Header().Set("Content-Type", "application/connect+json")`);
        server.push(`\t\t\tw.Header().Set("Connect-Protocol-Version", "1")`);
        server.push(`\t\t\tw.WriteHeader(http.StatusOK)`);
        server.push(`\t\t\tflusher.Flush()`);
        server.push(`\t\t\tsend := func(msg *${resName(rpc)}) error {`);
        server.push(`\t\t\t\tpayload, _ := json.Marshal(msg)`);
        server.push(`\t\t\t\tw.Write(speconn.EncodeEnvelope(0, payload))`);
        server.push(`\t\t\t\tflusher.Flush()`);
        server.push(`\t\t\t\treturn nil`);
        server.push(`\t\t\t}`);
        server.push(`\t\t\tif err := svc.${expName}(&req, send); err != nil {`);
        server.push(`\t\t\t\tendPayload, _ := json.Marshal(map[string]string{"error": err.Error()})`);
        server.push(`\t\t\t\tw.Write(speconn.EncodeEnvelope(speconn.FlagEndStream, endPayload))`);
        server.push(`\t\t\t} else {`);
        server.push(`\t\t\t\tendPayload, _ := json.Marshal(map[string]any{})`);
        server.push(`\t\t\t\tw.Write(speconn.EncodeEnvelope(speconn.FlagEndStream, endPayload))`);
        server.push(`\t\t\t}`);
        server.push(`\t\t\tflusher.Flush()`);
      } else {
        server.push(`\t\t\tvar req ${reqName(rpc)}`);
        server.push(`\t\t\tif r.ContentLength > 0 { json.NewDecoder(io.LimitReader(r.Body, r.ContentLength)).Decode(&req) }`);
        server.push(`\t\t\tres, err := svc.${expName}(&req)`);
        server.push(`\t\t\tif err != nil {`);
        server.push(`\t\t\t\tcode := speconn.CodeInternal`);
        server.push(`\t\t\t\tmsg := err.Error()`);
        server.push(`\t\t\t\tif se, ok := err.(*speconn.SpeconnError); ok { code = se.Code; msg = se.Message }`);
        server.push(`\t\t\t\tw.Header().Set("Content-Type", "application/json")`);
        server.push(`\t\t\t\tw.WriteHeader(code.HTTPStatus())`);
        server.push(`\t\t\t\tjson.NewEncoder(w).Encode(map[string]string{"code": string(code), "message": msg})`);
        server.push(`\t\t\t\treturn`);
        server.push(`\t\t\t}`);
        server.push(`\t\t\tw.Header().Set("Content-Type", "application/json")`);
        server.push(`\t\t\tjson.NewEncoder(w).Encode(res)`);
      }
    }
    server.push(`\t\tdefault:`);
    server.push(`\t\t\thttp.NotFound(w, r)`);
    server.push(`\t\t}`);
    server.push(`\t})`);
    server.push('}\n');
    ...
    client.push('import "github.com/speconn/speconn-go/speconn"\n');
